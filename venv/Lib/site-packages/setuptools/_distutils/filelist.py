...
class FileList:
    ...
    def _parse_template_line(self, line):
        ...
        if action in ('includes', 'exclude', 'global-includes', 'global-exclude'):
            if len(words) < 2:
                raise DistutilsTemplateError(
                    "'%s' expects <pattern1> <pattern2> ..." % action
                )
            patterns = [convert_path(w) for w in words[1:]]
        elif action in ('recursive-includes', 'recursive-exclude'):
            if len(words) < 3:
                raise DistutilsTemplateError(
    ...
    def process_template_line(self, line):  # noqa: C901
        ...
        if action == 'includes':
            self.debug_print("includes " + ' '.join(patterns))
        ...
        elif action == 'global-includes':
            self.debug_print("global-includes " + ' '.join(patterns))
            for pattern in patterns:
        ...
        elif action == 'recursive-includes':
            self.debug_print("recursive-includes {} {}".format(dir, ' '.join(patterns)))
            for pattern in patterns:
