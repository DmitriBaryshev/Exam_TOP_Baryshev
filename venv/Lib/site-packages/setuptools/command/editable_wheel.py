...
_LENIENT_WARNING = """
Options like `package-data`, `includes/exclude-package-data` or
`packages.find.exclude/includes` may have no effect.
"""
...
def _find_virtual_namespaces(pkg_roots: Dict[str, str]) -> Iterator[str]:
    ...
    phase (e.g. ``find_packages(includes=["mypkg.*"])``, when ``mypkg.foo`` is included
