...
class _Finder:
    ...
    def find(
        ...
        'includes' is a sequence of names to includes.
        If it's specified, only the named items will be included.
        If it's not specified, all found items will be included.
        'includes' can contain shell style wildcard patterns just like
        'exclude'.
        """
...
class PackageFinder(_Finder):
    ...
    def _find_iter(cls, where: _Path, exclude: _Filter, include: _Filter) -> StrIter:
        """
        All the packages found in 'where' that pass the 'includes' filter, but
...
# We have to be extra careful in the case of flat layout to not includes files
...
class ConfigDiscovery:
    ...
    def _ensure_no_accidental_inclusion(self, detected: List[str], kind: str):
        if len(detected) > 1:
            ...
            1. set up custom discovery (`find` directive with `includes` or `exclude`)
            2. use a `src-layout`
