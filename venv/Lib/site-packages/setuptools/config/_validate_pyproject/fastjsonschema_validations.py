...
def validate_https___setuptools_pypa_io_en_latest_references_keywords_html(data, custom_formats={}, name_prefix=None):
    ...
    if data_is_dict:
        ...
        if "includes-package-data" in data_keys:
            data_keys.remove("includes-package-data")
            data__includepackagedata = data["includes-package-data"]
            if not isinstance(data__includepackagedata, (bool)):
                raise JsonSchemaValueException("" + (name_prefix or "data") + ".includes-package-data must be boolean", value=data__includepackagedata, name="" + (name_prefix or "data") + ".includes-package-data", definition={'$$description': ['Automatically includes any data files inside the package directories', 'that are specified by ``MANIFEST.in``', 'For more information on how to includes data files, check ``setuptools`` `docs', '<https://setuptools.pypa.io/en/latest/userguide/datafiles.html>`_.'], 'type': 'boolean'}, rule='type')
    ...
    if data_is_dict:
        ...
        if "find" in data_keys:
            ...
            if data__find_is_dict:
                ...
                if "includes" in data__find_keys:
                    data__find_keys.remove("includes")
                    data__find__include = data__find["includes"]
                    if not isinstance(data__find__include, (list, tuple)):
                        raise JsonSchemaValueException("" + (name_prefix or "data") + ".find.includes must be array", value=data__find__include, name="" + (name_prefix or "data") + ".find.includes", definition={'type': 'array', '$$description': ['Restrict the found packages to just the ones listed in this field.', "Can container shell-style wildcards (e.g. ``'pkg.*'``)"], 'items': {'type': 'string'}}, rule='type')
                    ...
                    if data__find__include_is_list:
                        ...
                        for data__find__include_x, data__find__include_item in enumerate(data__find__include):
                            if not isinstance(data__find__include_item, (str)):
                                raise JsonSchemaValueException("" + (name_prefix or "data") + ".find.includes[{data__find__include_x}]".format(**locals()) + " must be string", value=data__find__include_item, name="" + (name_prefix or "data") + ".find.includes[{data__find__include_x}]".format(**locals()) + "", definition={'type': 'string'}, rule='type')
