...
def dict_of(key: ParserElement, value: ParserElement) -> ParserElement:
    ...
    can includes delimiting markers or punctuation, as long as they are
    suppressed, thereby leaving the significant key text.  The value
    pattern can includes named results, so that the :class:`Dict` results
    can includes named token fields.
